...
import { Benchmark, BenchmarkResult } from "./load.ts";
import { Config } from "./config.ts";
import { DEFAULT_INDEX_HTML } from "./default_index_html.ts";
import { leavePRComment } from "./comment/leavePRComment.ts";
import { leaveCommitComment } from "./comment/leaveCommitComment.ts";
...
async function loadListing(
  ...
  try {
    ...
    console.log(
      `Could not find listing.json at ${dataPath}. Using empty default: ${err}`,
    );
    return { ...DEFAULT_LISTING };
  }
}

async function updateAndStoreListing(
  baseDir: string,
  listingPath: string,
  data: Listing,
  isPr: boolean,
...
async function addIndexHtmlIfNeeded(
  additionalGitArguments: string[],
  baseDir: string,
  indexHtmlRelativePath: string,
) {
  const indexHtmlFullPath = path.join(baseDir, indexHtmlRelativePath);
  try {
    await fs.stat(indexHtmlFullPath);
    core.debug(
      `Skipped to create default index.html since it is already existing: ${indexHtmlFullPath}`,
    );
    return;
  } catch (_) {
    // Continue
...
function findAlerts(
  ...
  core.debug(
    `Comparing current:${curSuite.commit.id} and prev:${prevSuite.commit.id} for alert`,
  );

  const alerts: Alert[] = [];
  for (const current of curSuite.benches) {
    const currentKey = benchmarkKey(current, schema);
    const prev = prevSuite.benches.find(
      (b: BenchmarkResult) => benchmarkKey(b, schema) === currentKey,
    );
    if (prev === undefined) {
      core.debug(
        `Skipped because benchmark '${currentKey}' is not found in previous benchmarks`,
      );
      continue;
    }
...
function commentFooter(): string {
  ...
  const actionUrl =
    ...
    encodeURIComponent(github.context.workflow);
...
async function handleComment(
  ...
  if (!githubToken) {
    throw new Error(
      "'comment-always' input is set but 'github-token' input is not set",
    );
  }
...
async function handleAlert(
  ...
  if (!commentOnAlert && !failOnAlert) {
    core.debug(
      "Alert check was skipped because both comment-on-alert and fail-on-alert were disabled",
    );
    return;
  }
  ...
  if (commentOnAlert) {
    if (!githubToken) {
      throw new Error(
        "'comment-on-alert' input is set but 'github-token' input is not set",
      );
    }
  ...
  if (failOnAlert) {
    ...
    if (failures.length > 0) {
      core.debug(
        "Mark this workflow as fail since one or more fatal alerts found",
      );
...
function isRemoteRejectedError(err: unknown): err is Error {
  if (err instanceof Error) {
    return ["[remote rejected]", "[rejected]"].some((l) =>
      err.message.includes(l),
    );
  }
  return false;
}
...
async function writeBenchmarkToGitHubPagesWithRetry(
  ...
  if (githubToken && !skipFetchGhPages && ghRepository) {
    ...
    extraGitArguments = [
      `--work-tree=${benchmarkBaseDir}`,
      `--git-dir=${benchmarkBaseDir}/.git`,
    ];
  ...
  await git.cmd(
    ...
    "commit",
    "-m",
    `add ${name} benchmark result for ${bench.commit.id}`,
  );
  ...
  if (githubToken && autoPush) {
    try {
      ...
      core.warning(
        `Auto-push failed because the remote ${ghPagesBranch} was updated after git pull`,
      );
      ...
      if (retry > 0) {
        ...
        core.warning(
          `Failed to add benchmark data to '${name}' data: ${JSON.stringify(bench)}`,
        );
        throw new Error(
...
async function writeBenchmarkToExternalJson(
  ...
  const prevBench = addBenchmarkToDataJson(
    groupBy,
    schema,
    name,
    bench,
  ...
  try {
    ...
    throw new Error(
      `Could not store benchmark data as JSON at ${jsonFilePath}: ${err}`,
    );
  }
...
export async function writeBenchmark(bench: Benchmark, config: Config) {
  ...
  if (externalDataJsonPath) {
    prevBench = await writeBenchmarkToExternalJson(
      bench,
  ...
  if (!prevBench) {
    core.debug(
      "Alert check was skipped because previous benchmark result was not found",
    );
  } else {
...
export async function writeMetadata() {
  ...
  if (!repoUrl) {
    core.setFailed("Could not get repository URL from context.");
    return;
  }
  ...
  const shortSha = execSync(`git rev-parse --short ${commitSha}`).toString().trim();

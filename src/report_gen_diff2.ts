 ...
 * aggregates the results for the two most recent commits, and outputs a

 * Markdown-formatted comparison report to stdout. The report contains tables
 * that are grouped based on specified properties.
 *
 * @usage
 * ```
...
async function main() {
  ...
  if (!history || history.length < 1) {
    console.log(`## ⚠️ No Benchmark Data for ${args.name}`);
    Deno.exit(0);
  }

  // 2. UPDATED LOGIC: Find and aggregate data for the last two unique commits.
...
function generateMarkdown(
  ...
  for (const [groupKey, results] of groupedResults.entries()) {
    if (groupByKeys.length > 0 && groupKey !== "all") {
